...
def notify(cp: str, role: str, status: str, summary: str, result_file: str, config: dict):
    ...
    title = f"[SLv2] {cp} | {role_label} | {status} {emoji}"
...
def main():
    ...
    parser.add_argument(
        "--skip-ntfy", action="store_true",
        help="Do not send ntfy notification; useful when you only want DB/web sync"
    )
    parser.add_argument(
        "--skip-dashboard-post", action="store_true",
        help="Do not POST status to dashboard after notification"
    )
    parser.add_argument(
        "--dry-run", action="store_true",
        help="Print actions without sending notification or posting dashboard status"
    )
    ...
    if args.dry_run:
        ...
        print(f"  Notify: {'no' if args.skip_ntfy else 'yes'}")
        print(f"  Post dashboard: {'no' if args.skip_dashboard_post else 'yes'}")
        return
    ...
    if not args.skip_ntfy:
        ...
        print("\nSkipping ntfy notification (--skip-ntfy)")

    if not args.skip_dashboard_post:
        ok = post_dashboard_status(result_file, config)
        if not ok:
            sys.exit(1)
    else:
        print("Skipping dashboard sync (--skip-dashboard-post)")
